...
export const homeReducer = (state = initialState, action: { type: string }) => {
  ...
  switch (action.type) {
    case FILTER_FAVORITES:
      return {
        ...state,
        info: [...initialState.info].filter((item) => item.favorite)
      };
    case SORT_BY_NEW:
      return {
        ...state,
        info: [...initialState.info].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())
      };
    case SORT_BY_POPULAR:
      return {
        ...state,
        info: [...initialState.info].sort((a, b) => b.popular - a.popular)
      };
    default:
      return state;
  };
